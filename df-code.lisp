...
(defun matgloss-target (type subtype)
  (let* ((raws $global.world.raws)
         (key (enum-to-key $item_type type))
         (table (case key
                  ($WOOD $raws.matgloss.wood)
                  ($STONE_GRAY $raws.matgloss.stone)
                  ($STONE_LIGHT $raws.matgloss.stone)
                  ($STONE_DARK $raws.matgloss.stone)
                  ($GEM_ORNAMENTAL $raws.matgloss.gem)
                  ($GEM_SEMI $raws.matgloss.gem)
                  ($GEM_PRECIOUS $raws.matgloss.gem)
                  ($GEM_RARE $raws.matgloss.gem)
                  ($BONE $raws.creatures)
